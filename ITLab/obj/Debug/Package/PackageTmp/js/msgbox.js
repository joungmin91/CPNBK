...
(function ($) {
    var isPhone = navigator.userAgent.match(/android|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|meego.+mobile|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows (phone)|xda|xiino|playbook|silk/i) != null;
        ...
        MsgBoxObject: {
            ...
            show: function (txt, options, callback) {
                ...
                if (isPhone) {
                    ...
                    if (options.type.search(/alert$|info$|error$/i) >= 0) alert(txt);
                    ...
                    if (options.type === 'confirm') {
                        if ($.isFunction(callback)) {
                            if (confirm(txt) == true) callback('OK');
                            else callback(false);
                        } else confirm(txt);
                    }
                    ...
                    if (options.type != 'prompt') {
                        $.each(buttons, $.proxy(function (i, button) {
                            if (button.type == 'submit') {
                                this.esqueleto.buttons.append($('<button type="submit" autofocus>' + button.value + '</button>').bind('click', $.proxy(function (e) { this.close(button.value); e.preventDefault(); }, this)));
                            }
